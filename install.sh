...
read_choice() {
    local prompt="$1"
    local choice
    # Display prompt to stderr so it doesn't get captured
    echo -n -e "${GREEN}${prompt}${NC}" >&2
    # Read user input
    read -r choice
    # Trim whitespace and return via stdout
    echo "$choice" | xargs
}
...
main() {
    ...
    # Don't download immediately - wait until user needs it
    ADDON_DIR=""
    ...
    while true; do
        ...
        # echo "DEBUG: Choice entered: [$choice]" >&2
        ...
        case "$choice" in
            ...
            2)
                ...
                # Download addons if not already downloaded
            ...
            *)
                ...
                print_warning "You entered: '$choice'"
                press_any_key
                ;;
        esac
    done
